  ...
  <div class="trade-container">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span v-if="userDefaultAddress == undefined">四川</span>
        <span v-else>{{ userDefaultAddress.fullAddress }}</span>
        收货人：<span v-if="userDefaultAddress == undefined">邹昆宏</span>
        <span v-else>{{ userDefaultAddress.consignee }}</span>
        <span v-if="userDefaultAddress == undefined">17780297834</span>
        <span v-else>{{ userDefaultAddress.phoneNum }}</span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    changeDefault(address, addressInfo) {
      //全部的isDefault为零
      addressInfo.forEach((element) => (element.isDefault = 0));
      address.isDefault = 1;
    },
